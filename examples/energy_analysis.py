...
from pyspark.sql.functions import col, when, avg, max as spark_max, min as spark_min, round as spark_round
...
def analyze_production_vs_shortage_status(spark, api_key, hours=6):
    ...
    try:
        ...
        df_combined = df_production.alias("prod").join(
            df_shortage.alias("short"),
            col("prod.startTime") == col("short.startTime"),
            "inner"
        ).select(
        ...
        status_stats = df_combined.groupBy("shortage_status", "shortage_status_description") \
            .agg(
        ...
        df_with_features = df_combined.withColumn(
            "hour", F.hour("timestamp")
        ).withColumn(
            "production_change",
            col("production_mw") - F.lag("production_mw").over(Window.orderBy("timestamp"))
        )

        # Hourly averages
        hourly_avg = df_with_features.groupBy("hour") \
            .agg(
            spark_round(avg("production_mw"), 2).alias("avg_production_mw"),
        ...
        low_production = df_combined.filter(col("production_mw") < 6000) \
        ...
        shortage_distribution = df_combined.groupBy("shortage_status_description") \
...
def analyze_consumption_patterns(spark, api_key, hours=24):
    ...
    try:
        ...
        df_consumption.select(
            "startTime", "consumption_kwh", "time_series_type",
            "resolution", "unit", "measurement_count"
        ...
        df_consumption_mwh = df_consumption.withColumn(
            "consumption_mwh", spark_round(col("consumption_kwh") / 1000, 2)
        )

        # Hourly analysis
        df_hourly = df_consumption_mwh.withColumn(
            "hour", F.hour("startTime")
        )

        hourly_consumption = df_hourly.groupBy("hour") \
            .agg(
        ...
        peak_analysis = df_with_peak.groupBy("period_type") \
            .agg(
            spark_round(avg("consumption_mwh"), 2).alias("avg_consumption_mwh"),
...
def create_energy_dashboard_data(spark, api_key):
    ...
    try:
        ...
        df_aggregated = df_production.withColumn(
            "interval_start",
            F.date_trunc("hour", "startTime") +
            F.expr("INTERVAL 15 MINUTES") *
            F.floor(F.minute("startTime") / 15)
...
def main():
    ...
    spark = SparkSession.builder \
        .appName("Fingrid Energy Analysis") \
    ...
    dashboard_data = create_energy_dashboard_data(spark, api_key)
